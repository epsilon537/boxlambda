...
: bitmap-1bpp-test
  ...
  [:
    ...
    [:
      ...
      ts pixel{ 0 tidx 0 0 vec2 xy #101 color }set
      ts pixel{ w @ 1- 0 vec2 xy }set
      ts pixel{ 0 h @ 1- vec2 xy }set
      ts pixel{ w @ 1- h @ 1- vec2 xy }set
      0 irq :: irqline-set
      ...
      w @ 2- line-capture :: pxl@ hex. cr
      w @ 1- line-capture :: pxl@ hex. cr
      ts pixel{ 0 0 vec2 xy 0 color }set
      ts pixel{ w @ 1- 0 vec2 xy }set
      ts pixel{ 0 h @ 1- vec2 xy }set
      ts pixel{ w @ 1- h @ 1- vec2 xy }set
    ;] iter
