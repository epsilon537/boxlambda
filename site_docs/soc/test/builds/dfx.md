...
- [j1b_cli](https://github.com/epsilon537/boxlambda/blob/master/sw/projects/test/dfx_test/j1b_cli.cpp): This CLI allows you to boot the SwapForth firmware image on the J1B core and to transfer serial port I/O to the J1B providing access to its REPL.
...
- [mem_fs_cli](https://github.com/epsilon537/boxlambda/blob/master/sw/components/test/mem_fs_cli/mem_fs_cli.cpp): This CLI module provides file system commands such as `ls` and `rm` as well as commands for loading files into memory and save memory buffers to files.
