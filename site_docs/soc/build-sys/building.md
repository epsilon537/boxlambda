...
See [here](build-sys.md#four-layers) for more info on the `gw_codegen` target.
...
1. *Make* triggers the `gw_codegen` rule, executing all dependent gateware code generation *custom commands* (See also [this section](build-sys.md#four-layers)). **Note that because the code generators are always re-run, a `make \*_synth` build always rebuilds.**
