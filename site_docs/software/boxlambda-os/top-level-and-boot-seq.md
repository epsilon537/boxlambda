...
**BoxLambda OS Software Project top-level in the BoxLambda Directory Tree**: [sw/projects/boxlambda_os](https://github.com/epsilon537/boxlambda/blob/v0.4.1/sw/projects/boxlambda_os/main.cpp)
...
2. `main()` performs the following steps:
    ...
    3. Evaluate Forth module [fs/forth/early.fs](https://github.com/epsilon537/boxlambda/blob/v0.4.1/fs/forth/early.fs). This module contains some early definitions extending the core Word set. Currently, only the Word `c-fun` is defined here.
    4. Redirect the C library's stdio to Forth's `emit` and `key` Words. See [stdio_redirect.cpp](https://github.com/epsilon537/boxlambda/blob/v0.4.1/sw/components/forth/stdio_redirect_ffi.cpp).
    5. Initialize the Forth<->C Filesystem Foreign Function Interface (FFI). See [fs_ffi.cpp](https://github.com/epsilon537/boxlambda/blob/v0.4.1/sw/components/forth/fs_ffi.cpp).
    6. Evaluate the Forth modules listed in [fs/forth/boxkern-includes.fs](https://github.com/epsilon537/boxlambda/blob/v0.4.1/fs/forth/boxkern-includes.fs).
    7. Transfer control to Forth by evaluating [fs/forth/init.fs](https://github.com/epsilon537/boxlambda/blob/v0.4.1/fs/forth/init.fs). `init.fs` is expected to invoke the `quit` Word, starting the Forth REPL.
...
[fs/forth/boxkern-includes.fs](https://github.com/epsilon537/boxlambda/blob/v0.4.1/fs/forth/boxkern-includes.fs) may look like a Forth module but it's not.
