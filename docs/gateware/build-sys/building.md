...
See [here](build-struct.md#four-layers) for more info on the `gw_codegen` target.
...
![Gateware Build Targets.](../../assets/gw-build-targets.drawio.png)

*Gateware Build Targets.*

When you run `make hello_world_synth`, the following steps occur:

1. *Make* triggers the `gw_codegen` rule, executing all dependent gateware code generation *custom commands* (See also [this section](build-struct.md#four-layers)). **Note that because the code generators are always re-run, a `make \*_synth` build always rebuilds.**
...
![Merging a .mem file into a bitstream file.](../../assets/merge-mem-file-into-bitstream-file.drawio.png)
