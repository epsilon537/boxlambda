...
The toolchain tarball is checked into the BoxLambda repo under `assets/`. The [boxlambda_setup.sh](../../boxlambda_setup.sh) script unpacks the toolchain tarball in the `tools/` directory, so the user no longer needs to provide the toolchain as a prerequisite.
