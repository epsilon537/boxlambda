////////////////////////////////////////////////////////////////////////////////
//
// Epsilon: This version of flashdrvr has some minor modifications
// for BoxLambda. These are annotated with "BoxLambda".
// Note that BoxLambda only uses flashsim in single SPI mode.
//
...
int FLASHSIM::load(const unsigned addr, const char *fname) {
    FILE    *fp;
    size_t    len;
    int    nr = 0;
...
int    FLASHSIM::operator()(const int csn, const int sck, const int dat) {
    ...
    //BoxLambda: if we get here, CS_n is asserted.
    ...
    //SCK Rising edge - shift in miso
    else if (!m_last_sck && sck) {
      if (m_mode == FM_QSPI) {
      ...
      if (m_state == QSPIF_QUAD_READ_IDLE) {
          ...
          case 0x9f: // Read ID
              m_state = QSPIF_RDID;
              if (m_debug) printf("FLASHSIM: READING ID, 0x%x\n", DEVID);
          ...
          case QSPIF_SECTOR_ERASE:
              if (m_count == 32) {
                  ...
                  //BoxLambda: disabling this assert. Not sure why this was here. m_addr can go up to 0x00ffffff (24MB).
