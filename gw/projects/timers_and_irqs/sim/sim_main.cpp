...
int main(int argc, char** argv, char** env) {
    ...
    while (interactive_mode || (contextp->time() < 40000000)) {
      ...
      if (contextp->time() == 5000000) {
        printf("SIM: inserting uart character (1).\n");
        //Enter a character into the UART
        uart->enterCharInTxPath('a');
      }

      if (contextp->time() == 6000000) {
        printf("SIM: inserting uart character (2).\n");
        //Enter a character into the UART
        uart->enterCharInTxPath('b');
      }

      if ((uartCharsInserted==0) && (contextp->time() >= 7000000)) {
        //Enter characters into the UART
      ...
      if ((uartCharsInserted==1) && (contextp->time() >= 13000000)) {
        //Enter characters into the UART
        if (uart->enterCharInTxPath(*strPtr) != -1) {
          if (strPtr == strPtrEnd) {
