...
static void tick(void) {
  //Tick twice when Input clock is 100MHz and BoxLambda's system clock runs at 50MHz.
  //->Advance two input clock cycles at a time.
  //Tick four times when Input clock is 100MHz and BoxLambda's system clock runs at 25MHz.
  //->Advance four input clock cycles at a time.
  for (int ii=0; ii<(top->sys_clk_25mhz ? 4 : 2); ii++) {
    //High phase
    top->clk_i = 1;
    contextp->timeInc(1);
    top->eval();
...
int main(int argc, char** argv, char** env) {
    ...
    // When not in interactive mode, simulate for 4000000 timeprecision periods
    while (interactive_mode || (contextp->time() < 4000000)) {
      // Evaluate model
      tick();
    }
