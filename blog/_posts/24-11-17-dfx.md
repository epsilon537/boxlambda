...
Recap
-----
![BoxLambda Block Diagram.](../assets/dfx/Arch_Diagram_DFX_Focus.png)
...
The BoxLambda SoC top-level: [gw/components/boxlambda_soc/rtl/boxlambda_soc.sv](https://github.com/epsilon537/boxlambda/blob/dfx/gw/components/boxlambda_soc/rtl/boxlambda_soc.sv)
...
![DFX Terminology applied to the BoxLambda SoC](../assets/dfx/dfx_terminology.png)
...
![The DFX Test Program](../assets/dfx/dfx_test_program.png)
...
![The J1B Core fitted to the VS0 interface](../assets/dfx/vs0_j1b.png)
...
![The j1b_test SoC](../assets/dfx/j1b_test_soc.png)
...
[https://github.com/epsilon537/boxlambda/tree/dfx/gw/projects/j1b_test](https://github.com/epsilon537/boxlambda/tree/dfx/gw/projects/j1b_test)

**The j1b_test Test Program**:

[https://github.com/epsilon537/boxlambda/blob/dfx/sw/projects/j1b_test/j1b_test.cpp](https://github.com/epsilon537/boxlambda/blob/dfx/sw/projects/j1b_test/j1b_test.cpp)

Phase 2: Defining the RP and RMs
--------------------------------

![The BoxLambda SoC with RP vs0_inst and RMs vs0_stub and vs0_j1b](../assets/dfx/soc_w_rp_vs0.png)
...
![The vs0_stub](../assets/dfx/vs0_stub.png)
...
Empty vs0 module declaration (In practice, I'm using the [vs0_stub source](https://github.com/epsilon537/boxlambda/blob/dfx/gw/components/vs0_stub/rtl/vs0.sv) with the module body *ifdef'd* out):
...
![Pblock with properties](../assets/dfx/pblock_w_properties.png)
...
3. Draw a rectangular region. Keep an eye on the number of selected resources (LUTs, BRAM, etc.) and make sure there are enough resources to accommodate the biggest RM variant for this RP.
    ...
    ![Pblock with resources](../assets/dfx/pblock_w_resources.png)
...
- **Step 1**: Project Synthesis:
    ...
    - **Vivado Scripts Invoked**: [scripts/vivado_create_project.tcl](https://github.com/epsilon537/boxlambda/blob/dfx/scripts/vivado_create_project.tcl) followed by [scripts/vivado_synth.tcl](https://github.com/epsilon537/boxlambda/blob/dfx/scripts/vivado_synth.tcl).
- **Step 2**: Component OOC Synthesis:
    ...
    - **Vivado Scripts Invoked**: [scripts/vivado_create_project.tcl](https://github.com/epsilon537/boxlambda/blob/dfx/scripts/vivado_create_project.tcl) followed by [scripts/vivado_synth.tcl](https://github.com/epsilon537/boxlambda/blob/dfx/scripts/vivado_synth.tcl) (same scripts as used for project synthesis, but the *create_project* script is invoked with a *-ooc* flag).
- **Steps 3-7**: Building the project's static bitstream file as well as the bitstream file of the default RM:
    ...
    - **Vivado Script Invoked**: [scripts/vivado_impl_dfx_prj.tcl](https://github.com/epsilon537/boxlambda/blob/dfx/scripts/vivado_impl_dfx_prj.tcl)
- **Steps 8-10**: Building the alternate RM's bitstream file:
    - **Directory**: *build/arty-a7-100t/gw/components/\<component>*
    - **Build Command**: `make <component_name>_bit`
    - **Vivado Script Invoked**: [scripts/vivado_impl_dfx_rm.tcl](https://github.com/epsilon537/boxlambda/blob/dfx/scripts/vivado_impl_dfx_rm.tcl).
...
1. A software program loads the RM's partial bitstream file into a memory buffer.
    ...
    ![DFX Flow 1](../assets/dfx/dfx_flow_1.png)
...
2. The software program shuts down the DFX Controller, configures the address and size of the bitstream memory buffer in the DFX Controller's *BS INFO* registers, then re-enables the DFX Controller.
    ...
    ![DFX Flow 2](../assets/dfx/dfx_flow_2.png)
...
3. The software program issues a trigger to the DFX Controller using the *SW_TRIGGER* register. This causes the DFX Controller to start fetching the bitstream data from the configured memory location and feeding it to the internal configuration access port (ICAP).
    ...
    ![DFX Flow 3](../assets/dfx/dfx_flow_3.png)

4. When the RP is configured, the DFX Controller resets the RP/RM.

    ![DFX Flow 4](../assets/dfx/dfx_flow_4.png)
...
![DFX Controller Options](../assets/dfx/dfx_controller_options.png)
...
![DFX Controller IP and Register Map](../assets/dfx/dfx_controller_ip_addr_map.png)
...
![WB DFX Controller Block Diagram](../assets/dfx/wb_dfx_controller.png)

*Wb_dfx_controller block diagram.*

Here's the RTL:

[gw/components/wb_dx_controller/rtl/wb_dfx_controller.sv](https://github.com/epsilon537/boxlambda/blob/dfx/gw/components/wb_dfx_controller/rtl/wb_dfx_controller.sv)

The DFX Test Program Structure and CLI
--------------------------------------

![dfx test sw](../assets/dfx/dfx_test_sw.png)
...
- [dfx_cli](https://github.com/epsilon537/boxlambda/blob/dfx/sw/projects/dfx_test/dfx_cli.cpp): This is a CLI wrapper around the [dfx_hal](https://github.com/epsilon537/boxlambda/blob/dfx/sw/components/dfx/dfx_controller_hal.h) component. Most commands let you interact with the DFX Controller at a low level. However, there's one high-level command, **dfx_load_module**, that implements the entire sequence of loading an RM's bitstream file from the filesystem into the VS0 RP.
    ...
    * dfx_control
    ...
    ```
- [j1b_cli](https://github.com/epsilon537/boxlambda/blob/dfx/sw/projects/dfx_test/j1b_cli.cpp): This CLI allows you to boot the SwapForth firmware image on the J1B core and to transfer serial port I/O to the J1B providing access to its REPL.
...
- [mem_fs_cli](https://github.com/epsilon537/boxlambda/blob/dfx/sw/components/mem_fs_cli/mem_fs_cli.cpp): This CLI module provides file system commands such as *ls* and *rm* as well as commands for loading files into memory and save memory buffers to files.
...
The block diagram above shows a [vs0_hal](https://github.com/epsilon537/boxlambda/blob/dfx/sw/components/vs0_hal/vs0_hal.h). This HAL contains definitions that are common across all VS0 RM variants (currently just *vs0_j1b* and *vs0_stub*):
...
=============================================================
![pblock routing](../assets/dfx/pblock_routing.png)
...
![static design changes in pblock](../assets/dfx/static_design_changes_in_pblock.png)
...
Other Changes
-------------
Not all changes I make between blog posts are directly related to the topic at hand. In the past, I used to capture these updates here, but moving forward, I’ll track them in a dedicated [CHANGELOG.md](https://github.com/epsilon537/boxlambda/blob/dfx/CHANGELOG.md) file.
...
Setup
=====
1. Install the [Software Prerequisites](https://boxlambda.readthedocs.io/en/nov_17_24/prerequisites/).
...
1. Hook up the MicroSD PMOD as described [here](https://boxlambda.readthedocs.io/en/nov_17_24/pmods/#microsd-pmod) and insert a FAT formatted SD card.
