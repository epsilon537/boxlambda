...
- Test builds running on Arty-A7-35T and on Verilator.
...
LiteX and LiteDRAM
------------------
[LiteX](https://github.com/enjoy-digital/litex) is an Open Source SoC Builder framework for FPGAs. You specify which CPU, memory, interconnect, and periphals you want. The framework then generates the SoC for you, and the software to go along with it. Here's an example (with semi-randomly picked settings):
...
LiteX is written in [Migen](https://m-labs.hk/gateware/migen/), *a Python-based tool that automates further the VLSI design process*, to quote the website. At the heart of Migen sits [FHDL](https://m-labs.hk/migen/manual/fhdl.html), the Fragmented Hardware Description Language. FHDL is essentially a Python-based data-structure consisting of basic constructs to describe signals, registers, FSMs, combinatiorial logic, sequential logic etc. Here's an example:
...
You can more or less *see* the Verilog equivalent. However, the fact that this is a Python data structure means that you have Python at your disposal as a meta-language to combine and organize these bits of HDL. This is a huge increase in abstraction and expressiveness, and it explains how LiteX can do what it does. Litex provides a flexibility in mixing and matching of cores, core features, and interconnects that just can't be achieved with *vanilla* SystemVerilog.

These are powerful concepts. I'm tempted to jump onto the LiteX bandwagon, but I'm not going to. Not yet at least. BoxLambda is a journey, learning about FPGA development. I have a lot more learning to do before I can commit to a specific framework.

LiteX is not an all-or-nothing proposition, however. LiteX cores, such as the LiteDRAM memory controller, can be integrated into traditional design flows. That's what I'll be doing.
...
--------------------------
LiteDRAM is a highly configurable core (because Migen). For an overview of the core's features, please take a look at the LiteDRAM repository's README file:
...
You specify the configuration details in a .yml file. A Python script parses that .yml file and generates the core's Verilog as well as a CSR register access layer fro software.
...
- The *PHY layer*, *Electrical* and *Core* sections I left exactly as-is from the given Arty example.
- In the *General* section, I set *cpu* to *None*. BoxLambda already has a CPU. We don't need LiteX to generate one.
- In the *Frequency* section, I set *sys_clk_freq* to 50MHz. 50MHz has been the system clock frequency in the previous BoxLambda test builds as well, and I wasn't able to close timing at 100MHz. 
...
- A Wishbone control port is generated along with the two requested user ports. LiteDRAM CSR register access is done through the control port.
...
- The LiteDRAM module takes a 100MHz input clock (clk) and provides a 50MHz system clock (user_clk). The module contains a clock generator.
- On FPGA, the LiteDRAM module takes an asynchronous reset (rst) and provide a synchronized reset (user_rst). The module contains a reset synchronizer.
...
How long to STB?
================
One Rookie mistake I made early on was to just set *stall* to 0. I figured that, as long as I didn't generate multiple outstanding transactions, that should work just fine. That's not the case however. Wishbone transactions to the LiteDRAM core would block. The reason is that in classic Wishbone, STB has to remain asserted until an ACK or ERR is signalled by the slave. Pipelined Wishbone doesn't work that way. In pipelined Wishbone, as long as the slave is not stalling, for a single access STB only remains asserted for one clock cycle.

![Classic Wishbone transaction.](../assets/wishbone_classic_transaction.png){:class="img-responsive"}
...
Wishbone Issues: Core2WB and WB_Interconnect_SharedBus
======================================================
With the *litedram_wrapper* in place, Wishbone transactions still weren't working properly. The Wishbone Bus Master didn't behave like a proper Wishbone pipelined bus master should behave. That problem could either come from the Ibex core to Wishbone adapter, *core2wb.sv*, or the Wishbone shared bus implementation, *wb_interconnect_shared_bus.sv*, or both.
...
The job of *core2wb* is to adapt that interface to a pipelined Wisbhone bus master interface. That Wishbone bus master then requests access to the shared bus. It's up to *wb_interconnect_shared_bus* to grant the bus to one of the requesting bus masters and direct the transaction to the selected slave. If either one of those modules has a bug, that will result in an incorrectly behaving pipelined Wishbone bus master, from the point of view of a pipelined Wisbone slave.

![Ibex to LiteDRAM.](../assets/Ibex_to_LiteDRAM.drawio.png){:class="img-responsive"}
*From Ibex to LiteDRAM.*

*core2wb.sv* and *wb_interconnect_shared_bus.sv* are part of the *ibex_wb* repository. I looked long and hard at their implementation and ultimately decided that I couldn't figure out the author's reasoning. The *ibex_wb* repository no longer appears to be actively maintained, so I decided to replace both modules with my own implementation. 

- *Core2wb* has two states: *Idle* and *Transaction Ongoing*. In the Idle state, when Ibex signals a transaction request (core.req), the Ibex memory interface signals get registered, a single acces pipelined Wishbone transaction is generated and *core2wb* goes to *Transaction Ongoing* state. When a WB ACK or ERR response is received, core2wb goes back to idle. While *Transaction Ongoing* state, the memory interface grant (*gnt*) signal is held low, so further transaction requests are stalled until *core2wb* is idle again Multiple outstanding transactions are currently not supported. I hope to add that capability someday.

![Core2WB State Diagram.](../assets/core2wb_fsm.drawio.png){:class="img-responsive"}
*Core2WB State Diagram.* 

- *WB_interconnect_shared_bus* also has two states: In Idle state a priority arbiter monitors the CYC signal of participating Bus Masters. When onr or more Bus Masters assert CYC, the arbiter grants access to the lowest order Bus Master and goes to *Transaction Ongoing* state. When that Bus Master deasserts CYC again, we go back to Idle state. Slave selection and forwarding of WB signals is done with combinatorial logic.

![WB_Interconnect_Shared_Bus State Diagram.](../assets/wb_shared_bus_fsm.drawio.png){:class="img-responsive"}
...
*ddr_test_soc*
--------------
[<BoxLambda>/projects/ddr_test/rtl/ddr_test_soc.sv](https://github.com/epsilon537/boxlambda/blob/master/projects/ddr_test/rtl/ddr_test_soc.sv) has the test build's top-level. It's based on the previous test build's top-level, extended with the LiteDRAM wrapper instance. Note that Clock and Reset generation is now done by LiteDRAM. LiteDRAM accepts the external clock and reset signal and provides the system clock and synchronized system reset.
...
- FPGA: [https://github.com/epsilon537/boxlambda/tree/develop/components/litedram/arty/sw/include/generated](https://github.com/epsilon537/boxlambda/tree/develop/components/litedram/arty/sw/include/generated)
- Sim: [https://github.com/epsilon537/boxlambda/tree/develop/components/litedram/sim/sw/include/generated](https://github.com/epsilon537/boxlambda/tree/develop/components/litedram/sim/sw/include/generated)
  
The most relevant files are *csr.h* and *sdram_phy.h*. They contain the register definitiions and constants used by the memory initialization code. Unfortunately, these accessors are not the same for the FPGA and the simulated LiteDRAM cores. We're going to have to use separate software builds for FPGA and simulation.

*Sdram_phy.h* also contains a function called *init_sequence()*. This function gets invoked as part of a more elaborate initialziation sequence function called *sdram_init()*. *Sdram_init()* is *not* part of the generated code, however. It's part of *liblitedram*, which is part of the base Litex distribution:
...
I found that whole bit rather confusing, to be honest. Why provide only part of the initialization code in the generated software layer, and not the entire *sdram_init()* function? Maybe the additional initialization actions taken by sdram_init() are optional? I couldn't find any documentation about this. I just did what I though was best based on code reading.
...
cd boxlambda
```
   1. Switch to the enter_litedram tag: 
```
git checkout enter_litedram
```
   1. Set up the repository. This initializes the git submodules used and builds picolibc for BoxLambda: 
```
make setup
```
...
![ddr_test on Verilator](../assets/ddr_test_verilator.png){:class="img-responsive"}
...
make run
```
   1. Verify the test program's output in the terminal. You should see something like this:

![ddr_test on Arty - Putty Terminal](../assets/ddr_test_arty.png){:class="img-responsive"}

Other Changes
-------------

- I noticed belatedly that I didn't create a constraint for the tck JTAG clock so no timing analysis could be done on the JTAG logic.
  I added the following to the .xdc constraints file. Vivado timing analysis is much happier now.
...
- I have merged the developement branch to the master branch. Going forward, I intend to do that everytime I put down a 'release' label for a new Blog post.
  
Interesting Links
-----------------
[PICO-8](https://www.lexaloffle.com/pico-8.php): PICO-8 by Zep/Lexaoffle is an extremely well-designed fantasy console. So simple, yet so deep. Absolutely worth checking out!
