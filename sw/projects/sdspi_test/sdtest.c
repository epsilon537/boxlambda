...
//Count the number of 'busy' interrupts. This number will be verified in the
...
//
  ...
  disable_irq(IRQ_ID_SDSPI);

  printf("Test is complete. SDSPI IRQs received: %d\n", num_sdpsi_busy_irqs);

  //Sucess if we get here and more than 20 busy irqs have been counted.
  return (num_sdpsi_busy_irqs > 20) ? 0 : -1;
}
