...
# catch this abort.
# Forth_Datastack FS_catch_evaluate(uint8_t* str, int count);
.global EvaluatState
EvaluateState:
  .word 0
EvaluateSP:
...
FS_catch_evaluate:
  ...
  laf x14, quit_evaluate # change quit hook
  laf x15, hook_quit
  sc x14, 0(x15)

  laf x14, EvaluateSP # save stackpointer
  sc  x2, 0(x14)

  laf x14, EvaluateState # set EvaluateState to 0
  sc  x0, 0(x14)

  call evaluate

  laf x14, quit_vanilla # restore quit hook
  laf x15, hook_quit
  sc x14, 0(x15)

  mv x10, x8 # return datastack
  mv x11, x9
...
quit_evaluate:
  li x14, 1
  laf x15, EvaluateState # set error state
  sc x14, 0(x15)
  ...
  laf x14, EvaluateSP # restore stackpointer
  lc  x2, 0(x14)
