...
opcodeentry_signed:
  ...
  # Check if the constant fits into a single opcode:
  li x15, -2048 # 0xFFFFF800
  and x14, x8, x15
  beq x14, x15,  opcodeentry_short
  bne x14, zero, opcodeentry_long

opcodeentry_short: # Short version with just one opcode.
...
opcodeentry_long: # Long version with two opcodes.
...
# -----------------------------------------------------------------------------
  Definition Flag_foldable_1|Flag_inline|Flag_noframe, "shr" # ( x -- x' ) # Move one position to the right
# -----------------------------------------------------------------------------
  srli x8, x8, 1
  ret

# -----------------------------------------------------------------------------
  Definition Flag_foldable_1|Flag_inline|Flag_noframe, "shl" # ( x -- x' ) # Move one position to the left
# -----------------------------------------------------------------------------
  slli x8, x8, 1
  ret
