    0xa08, 0xc09, 0xf0b};

Vera vera;

static void pixel_set_8bpp(uint8_t *base, uint32_t x, uint32_t y,
                           uint32_t width, uint8_t val) {
  base[y * width + x] = val;
}

static uint32_t pixel_get_8bpp(uint8_t *base, uint32_t x, uint32_t y,
                               uint32_t width) {
  return base[y * width + x];
}

static void pixel_set_4bpp(uint8_t *base, uint32_t x, uint32_t y,
                           uint32_t width, uint8_t val) {
...
static uint8_t pixel_get_4bpp(uint8_t *base, uint32_t x, uint32_t y,
                              uint32_t width) {
  uint8_t *p = base + (y * width + x) / 2;
  uint8_t b = *p;
...
static void pixel_set_2bpp(uint8_t *base, uint32_t x, uint32_t y,
                           uint32_t width, uint8_t val) {
...
static uint8_t pixel_get_2bpp(uint8_t *base, uint32_t x, uint32_t y,
                              uint32_t width) {
  uint8_t *p = base + (y * width + x) / 4;
  uint8_t b = *p;
  static const uint32_t bitoffsets[] = {6, 4, 2, 0};
...
static void pixel_set_1bpp(uint8_t *base, uint32_t x, uint32_t y,
                           uint32_t width, uint8_t val) {
...
static uint8_t pixel_get_1bpp(uint8_t *base, uint32_t x, uint32_t y,
                              uint32_t width) {
  uint8_t *p = base + (y * width + x) / 8;
  uint8_t b = *p;
  uint32_t bitoffset = 7 - (x & 7);
...
static void pixel_set(Vera_bpp_t bpp, uint8_t *base, uint32_t x, uint32_t y,
                      uint32_t width, uint8_t val) {
  switch (bpp) {
  case VERA_BPP_1:
...
static uint8_t pixel_get(Vera_bpp_t bpp, uint8_t *base, uint32_t x, uint32_t y,
                         uint32_t width) {
  switch (bpp) {
  case VERA_BPP_1:
...
uint32_t static map_size_enc(Vera_map_size_t sz) {
  ...
  case VERA_MAP_SZ_64:
    res = 1;
    break;
  case VERA_MAP_SZ_128:
    res = 2;
    break;
  case VERA_MAP_SZ_256:
    res = 3;
    break;
  default:
    assert(0);
...
uint16_t static tile_sz_enc(uint32_t sz) {
  uint16_t res;
...
bool Vera_map::init(Vera_map_size_t width, Vera_map_size_t height,
...
void Vera_map::write(uint32_t col, uint32_t row,
                     Vera_textmap_entry_16_t entry) {
  assert(initialized_);
...
void Vera_map::write(uint32_t col, uint32_t row,
                     Vera_textmap_entry_256_t entry) {
  assert(initialized_);
...
bool Vera_tileset::init(Vera_tile_size_t width, uint32_t height, Vera_bpp_t bpp,
                        uint32_t num_tiles) {
  assert(!initialized_);
  assert(num_tiles > 0);
...
uint8_t *Vera_tileset::tile_data(uint32_t tile_idx) {
  assert(initialized_);
  assert(tile_idx < num_tiles_);
  return tileset_base_ + tile_idx * tilesize_bytes_;
}

void Vera_tileset::pixel_set(uint32_t tile_idx, uint32_t x, uint32_t y,
                             uint8_t val) {
  assert(initialized_);
  assert(x < width_);
  assert(y < height_);

  uint8_t *tileptr = tile_data(tile_idx);
...
void Vera_sprite::tile_set(uint32_t tileset_id, uint32_t tile_idx) {
  ...
  uint8_t *addr = tileset.tile_data(tile_idx_);
...
void Vera_palette::restore_default() {
  for (int ii = 0; ii < 256; ii++)
    write(ii, default_palette[ii]);
}

Vera_palette::Vera_palette() { restore_default(); }
...
uint32_t Vera_layer::hscroll_get() { return vera_layer_regs_->HSCROLL; }
...
uint32_t Vera_layer::vscroll_get() { return vera_layer_regs_->VSCROLL; }
...
void Vera_layer::map_set(uint32_t map_id) {
  ...
  vera_layer_regs_->CONFIG_bf.T256C =
      (uint32_t)(map.map_type() == VERA_MAP_TYPE_TXT256);
...
void Vera_layer::tileset_set(uint32_t tileset_id) {
  ...
  vera_layer_regs_->TILEBASE_bf.TILE_BITMAP_WIDTH =
      (uint32_t)(tileset.width() == VERA_TILE_SZ_16);
  vera_layer_regs_->TILEBASE_bf.TILE_HEIGHT =
      (uint32_t)(tileset.height() == VERA_TILE_SZ_16);
  vera_layer_regs_->TILEBASE_bf.TILE_BASE_ADDR_16_11 =
      ((uint32_t)tileset.tileset_base() - VERA_VRAM_BASE) >> 11;
}

void Vera_layer::bitmap_set(uint32_t tileset_id, uint32_t tile_idx) {
  assert(tileset_id < VERA_NUM_TILESETS);
  assert(tile_idx != ~0U);
  ...
  vera_layer_regs_->TILEBASE_bf.TILE_BITMAP_WIDTH =
      (uint32_t)(tileset.width() == VERA_TILE_SZ_640);
  vera_layer_regs_->TILEBASE_bf.TILE_HEIGHT = 0;
  vera_layer_regs_->TILEBASE_bf.TILE_BASE_ADDR_16_11 =
      ((uint32_t)tileset.tile_data(tile_idx) - VERA_VRAM_BASE) >> 11;
}
...
void Vera_layer::set_id_(uint32_t layer) {
  layer_ = layer;
  vera_layer_regs_ =
      (Vera_layer_regs_t *)&(layer ? VERA->L1_CONFIG : VERA->L0_CONFIG);
  map_id_ = ~0U;
  tileset_id_ = ~0U;
  tile_idx_ = ~0U;
}
...
void Vera::display_enable(bool enable) {
  VERA->DC_VIDEO_bf.OUTPUT_MODE =
      enable ? VERA_DC_VIDEO_OUTPUT_MODE_VGA : VERA_DC_VIDEO_OUTPUT_MODE_DIS;
}
...
uint32_t Vera::alloc_(uint8_t *pool, uint32_t pool_size, uint32_t num_blocks) {
  ...
  while (block_idx < pool_size) {
    ...
    while ((jj < num_blocks) && (block_idx + jj < pool_size) &&
           (pool[block_idx + jj] == 0))
      ++jj;
    ...
    block_idx +=
        jj + 1; // Update block_idx so we continue the search where we left off.
  }
