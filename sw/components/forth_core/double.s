...
# -----------------------------------------------------------------------------
  ...
  addi x9, x9, -2*CELL # Two more elements

  sc x14, 0*CELL(x9)
  sc x14, 4*CELL(x9)

  lc x14, 3*CELL(x9)  # Old Position 4
  sc x14, 1*CELL(x9)
  sc x15, 2*CELL(x9)

  sc  x8, 3*CELL(x9)  # Old Position 4
...
udm_star: # Unsigned multiply 64*64 = 128
          # ( ud1 ud2 -- udl udh )
#------------------------------------------------------------------------------
  # On the data stack: ( 1L 1H 2L 2H -- LL  L  H HH )
  #                       12  8  4  0 after pushdatos
  #                        d  c  b  a    r0 r1 r2 r3
  # Need a long result register!
...
um_slash_mod: # ( ud u -- u u ) Dividend Divisor
             # 64/32 = 32 Remainder 32
#------------------------------------------------------------------------------
  push x1
  pushdaconst 0
  call ud_slash_mod
...
#------------------------------------------------------------------------------
  ...
  andi x8, x8, 2*CELLBITS-1
  beq x8, zero, dshift_drop

  addi x14, x8, -CELLBITS # Case differentiation, whether 32 or more positions are to be shifted
...
dlshift_short:  # Shift 31 or less

  lc  x15, 1*CELL(x9) # Low part first
  sll x14, x15, x8    # Low << n
  sc  x14, 1*CELL(x9)

  # Low included in x15, now tackle the high part:
...
#------------------------------------------------------------------------------
  ...
  lc x15, 0(x9)  # High part
  srl x15, x15, x14
  addi x9, x9, CELL
  sc x15, 0(x9)  # Save as low part
  li x8, 0
  ret

drshift_short:  # Shift 31 or less

  lc  x15, 0(x9)    # High-Part first
  srl x14, x15, x8  # High >> n
  sc  x14, 0(x9)

  # High contained in x15, now tackle the low part:
...
darshift_short:  # Shift 31 or less
