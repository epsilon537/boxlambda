...
int  sdcard_read_scr(unsigned *scr) {
  ...
  SDSPI->DAT = SECTOR_8B | SPEED_FAST;
  ...
  if (SDSPI->CMD & (SDSPI_ERROR | SDSPI_REMOVED)) {
    sdcard_err |= SDERR_INIT | 0x4000000;
    return -1;
  }

  SDSPI->DAT = 0x0;  // Read from position zero
  SDSPI->CMD = (SDSPI_CLEARERR|SDSPI_FIFO_OP|SDSPI_CMD)+51;
  SDSPI_WAIT_WHILE_BUSY;
...
int  sdcard_read_csd(char *csd) {
  ...
  //
  SDSPI->DAT = 0;
  SDSPI->CMD = (SDSPI_CLEARERR|SDSPI_FIFO_OP|SDSPI_CMD) + 9; // 0x08849;
  SDSPI_WAIT_WHILE_BUSY;
...
int  sdcard_read_cid(char *cid) {
  ...
  SDSPI->DAT = 0x0;
  SDSPI->CMD = (SDSPI_CLEARERR|SDSPI_FIFO_OP|SDSPI_CMD)+10; // 0x0984a;
  SDSPI_WAIT_WHILE_BUSY;

  if ((v = SDSPI->CMD) != 0x0) { // Expecting 0x0
...
int  sdcard_init(void) {
  ...
  SDSPI->DAT = SECTOR_512B | SPEED_SLOW;
  SDSPI->CMD = SDSPI_SETAUX;

  // Clear any prior pending errors
  SDSPI->DAT = 0;
  SDSPI->CMD = SDSPI_CLEARERR|SDSPI_READAUX;
  ...
  SDSPI->DAT = 0;
  SDSPI->CMD = SDSPI_GO_IDLE; // CMD zero
  SDSPI_WAIT_WHILE_BUSY;
  ...
  if (0) {
    ...
    SDSPI->DAT = 0x40000000;  // We support high capacity cards
    SDSPI->CMD = SDSPI_CMD+1;
    SDSPI_WAIT_WHILE_BUSY;
  ...
  SDSPI->DAT = 0x001a5;
  SDSPI->CMD = (SDSPI_CMD|SDSPI_READREG)+8;
  SDSPI_WAIT_WHILE_BUSY;
  ...
  {  int  dev_busy;
    ...
    do {
      ...
      //
      SDSPI->DAT = 0;
      SDSPI->CMD = SDSPI_ACMD;  // 0x70 = 'd55 + 'h40
      SDSPI_WAIT_WHILE_BUSY;

      //
      // Send the ACMD41 itself
      //
      SDSPI->DAT = 0x40000000; // Support high capacity
  ...
  //
  SDSPI->DAT = 0x0401;  // 16 byte block length, 25MHz clock
  // Write config data, read last config data
  SDSPI->CMD = SDSPI_SETAUX;
...
static  void  sdcard_get_r2(void) {
  if (SDDEBUG) {
    ...
    txstr("SD_WRITE.STAT: Status return, "); txhex(SDSPI->CMD);
    txstr(" : "); txhex(SDSPI->DAT);
    txstr("\r\n");

    sdcard_dump_r2(SDSPI->CMD);
  }
}
//
//
...
int  sdcard_read(int sector, char *buf) {
  ...
  //
  SDSPI->DAT = SECTOR_512B | SPEED_FAST;
  SDSPI->CMD = SDSPI_SETAUX;
...
int  sdcard_write(const int sector, const char *buf) {
  ...
  SDSPI->DAT = SECTOR_512B | SPEED_FAST;  // 128 word block length, 20MHz clock
